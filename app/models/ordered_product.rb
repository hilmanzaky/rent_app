...
#  end
  def generate_error_msg(err_msg)
    self.flash_error = "Gagal menyimpan data"

    if self.order_id.blank?
      return "/admin/products"    #      return "admin_products_path"
    else
      return "/admin/orders/#{self.order_id}/edit_detail"     #      return "admin_order_edit_detail_path"
    end
  end
